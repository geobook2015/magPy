...
def smooth(x, window_len=11, window='hanning'):
	...
	if not window in ['flat', 'hanning', 'hamming', 'bartlett', 'blackman','parzen']:
...
def smooth2d(x, window_len=11, window='hanning')	:
	...
	kernel = np.outer(signal.hanning(window_len[0], 8), signal.gaussian(window_len[1], 8))
	# pad to help the boundaries
	padded = np.pad(x, ((window_len[0], window_len[0]),(window_len[1], window_len[1])), mode="edge")
	# 2d smoothing
	blurred = signal.fftconvolve(padded, kernel, mode='same')
	print x.shape
	print padded.shape
	print blurred.shape
	return blurred[window_len[0]:window_len[0]+x.shape[0], window_len[1]:window_len[1]+x.shape[1]]
...
fx, xPower = signal.welch(x, fs, nperseg=windowSize, noverlap=0, scaling="spectrum")
fy, yPower = signal.welch(y, fs, nperseg=windowSize, noverlap=0, scaling="spectrum")
fxy, xyPower = signal.csd(x, y, fs, nperseg=windowSize, noverlap=0, scaling="spectrum")
...
for iW in xrange(0, windows):
	...
	for winSmooth in winSmoothers:
		for winType in winTypes:
			...
			absSmooth = smooth(np.absolute(arrPowXY[iW]), winSmooth, window=winType)
			testSmooth = np.absolute(smooth(arrPowXY[iW], winSmooth, window=winType))
			cohNom = np.power(testSmooth, 2)
			cohDenom = smooth(arrPowXX[iW], winSmooth, window=winType)*smooth(arrPowYY[iW], winSmooth, window=winType)
			coherence = cohNom/cohDenom
